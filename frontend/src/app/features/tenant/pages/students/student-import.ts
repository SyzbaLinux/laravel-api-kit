...
const CSV_HEADERS = ['name', 'email', 'phone', 'student_number', 'date_of_birth', 'gender', 'admission_date'];
const SAMPLE_CSV = `name,email,phone,student_number,date_of_birth,gender,admission_date
Alice Moyo,alice.moyo@school.edu,+263771234567,STU-001,2010-05-15,female,2024-01-15
Bob Ncube,bob.ncube@school.edu,,STU-002,2011-03-22,male,2024-01-15`;
...
@Component({
    ...
    <div class="p-6 lg:p-8">
      ...
      <div class="max-w-3xl">
        <div class="mb-6">
          <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Bulk Import Students</h1>
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Upload a CSV file to enrol multiple students at once.</p>
        </div>
        ...
        <div class="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-sm p-4 mb-6">
          ...
          <code class="block text-xs bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 p-2 rounded font-mono">
            name, email, phone, student_number, date_of_birth, gender, admission_date
          </code>
          <p class="text-xs text-blue-700 dark:text-blue-400 mt-2">
            <strong>Required:</strong> name, email &nbsp;&bull;&nbsp;
            <strong>Optional:</strong> all other fields &nbsp;&bull;&nbsp;
            <strong>Date format:</strong> YYYY-MM-DD &nbsp;&bull;&nbsp;
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                  @for (row of previewRows(); track $index) {
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                      ...
                      <td class="px-3 py-2 text-slate-500 dark:text-slate-400 font-mono">{{ row.student_number || '—' }}</td>
                      <td class="px-3 py-2 text-slate-500 dark:text-slate-400">{{ row.date_of_birth || '—' }}</td>
                      <td class="px-3 py-2 text-slate-500 dark:text-slate-400 capitalize">{{ row.gender || '—' }}</td>
                      <td class="px-3 py-2 text-slate-500 dark:text-slate-400">{{ row.admission_date || '—' }}</td>
                    </tr>
                  }
                </tbody>
              </table>
          ...
          <div class="mb-6 p-4 rounded-sm border"
            ...
            <div class="flex items-center gap-2 mb-2">
              @if (importResult()!.errors.length === 0) {
                ...
                <lucide-icon [img]="AlertTriangleIcon" [size]="16" class="text-yellow-600"></lucide-icon>
                <p class="text-sm font-semibold text-yellow-800">Import Complete with Warnings</p>
              }
            </div>
            <p class="text-sm text-slate-700">Successfully imported <strong>{{ importResult()!.imported }}</strong> student(s).</p>
            @if (importResult()!.errors.length > 0) {
              <ul class="mt-2 space-y-1">
                @for (err of importResult()!.errors; track $index) {
                  <li class="text-xs text-yellow-700">{{ err }}</li>
                }
              </ul>
            }
          </div>
...
export class StudentImport {
        ...
        reader.onload = (e) => {
            ...
            const rows: CsvRow[] = dataLines.slice(0, 5).map(line => {
                ...
                return {
                    ...
                    email: cols[1] ?? '',
                    phone: cols[2] ?? '',
                    student_number: cols[3] ?? '',
                    date_of_birth: cols[4] ?? '',
                    gender: cols[5] ?? '',
                    admission_date: cols[6] ?? '',
                };
            });
    ...
    onImport(): void {
        ...
        this.http.post<{ success: boolean; data: { imported: number; errors: string[] }; message: string }>(
            `${environment.apiUrl}/students/import`,
            formData,
        ).subscribe({
            next: (res) => {
